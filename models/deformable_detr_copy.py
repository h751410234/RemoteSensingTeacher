...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        if backbone_align:
            #---------fuse module
            self.f_attention = FSAS(256) #频域注意力
            self.norm_backbone = nn.LayerNorm(hidden_dim)

            self.leaarned_filter = nn.Parameter(torch.ones((1, 1024 // 2 + 1)))

            #DFFN
            #self.F_ffn = DFFN(hidden_dim)
            #norm FFN
            ...
            if self.backbone_align:
               ...
               # da_output['backbone'] = torch.cat([self.backbone_D(self.grl(src.flatten(2).transpose(1, 2))) for src in srcs], dim=1)
                ...
                for idx, src in enumerate(srcs):
                    ...
                    src_da = src_da + self.f_attention(sub_memory, src_da, sub_memory)  # change qkv

                    #---普通FFN-----
                    src_da__ = self.linear2(self.dropout3(self.activation(self.linear1(src_da))))
                    src_da = src_da + self.dropout4(src_da__)
                    src_da = self.norm3(src_da)
                    out_srcs_da.append(src_da)

                    # ---频域可学习滤波器----
                    # src_da__ = self.dropout3(self.activation(self.linear1(src_da)))
                    #
                    # src_da__ = torch.fft.rfft2(src_da__.float())
                    # src_da__ = src_da__ * self.leaarned_filter
                    # src_da__ = torch.fft.irfft2(src_da__)
                    #
                    # src_da__ = self.linear2(src_da__)
                    # src_da = src_da + self.dropout4(src_da__)
                    # src_da = self.norm3(src_da)
                    # out_srcs_da.append(src_da)
                    #
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes,SSOD = False):
        ...
        #---记录prob，自适应阈值使用
        prob = torch.gather(prob, 1, topk_boxes.unsqueeze(-1).repeat(1,1,4))
